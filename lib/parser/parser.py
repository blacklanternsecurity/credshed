...
class TextParse():
    ...
    def __init__(self, filename, unattended=False, strict=True, threshold=.80):
    ...
    def gather_info(self, num_lines=5000, skip_type_detection=False):
        ...
        if not self.filename.is_file():
            raise TextParseError(f'Failure reading {self.filename}')

        head = self._head(str(self.filename), num_lines=int(num_lines*2))
        ...
        elif self.num_input_fields == 5 and self.input_delimiter == b'\x00':
    ...
    def _get_delimiter(self, lines):
        ...
        try:
            ...
            raise DelimiterError(f'No delimiter candidates found in file "{self.filename}"')
        ...
        if self.unattended:
            ...
            if len(consistency_scores) > 1:
                ...
                if len(ties) > 1:
                    ...
                    raise DelimiterError(f'Multiple delimiter candidates: "{str(ties_str)[2:-1]}" in file "{self.filename}"')
            ...
            if valid_lines < self.threshold:
                raise DelimiterError('Delimiter "{}" ({:.1f}%) failed threshold ({:.1f}%) in "{}"'.format(\
                    str(d0[0])[2:-1], (valid_lines*100), self.threshold*100, self.filename))
    ...
    def __iter__(self):
        ...
        try:
            ...
            raise TextParsePermissionError(f'Error opening {self.filename}')
